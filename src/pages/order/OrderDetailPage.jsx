  ...
  return (
    <div className="">
      ...
      <button
        ...
        className="flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-navy mb-5 transition"
      >
        <IoArrowBack size={17} /> Back to Orders
      </button>

      <h1 className="text-xl font-extrabold text-navy mb-5">Order #{order.orderNumber}</h1>
      ...
      >
        <div className="w-14 h-14 rounded-xl bg-white/70 flex items-center justify-center flex-shrink-0">
          <IoBicycleOutline size={32} style={{ color: statusInfo.color || '#0D1B5E' }} />
        </div>
        <div>
          <p className="text-lg font-extrabold" style={{ color: statusInfo.color || '#0D1B5E' }}>
      ...
      <div className="flex flex-col gap-4">
        ...
        {order.status !== 'cancelled' && (
          <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            <h3 className="text-sm font-bold text-navy mb-4">Order Tracking</h3>
                  ...
                  <div key={step} className="flex items-stretch gap-4">
                    ...
                    <div className="flex flex-col items-center w-6">
                      <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 border-2 transition
                        ${isCompleted ? 'bg-navy border-navy' : isCurrent ? 'border-navy bg-white' : 'border-gray-300 bg-white'}`}>
                        {isCompleted && <IoCheckmark size={11} className="text-white" />}
                      </div>
                      {i < STATUS_STEPS.length - 1 && (
                        <div className={`w-0.5 flex-1 min-h-5 mt-0.5 rounded transition ${isCompleted ? 'bg-navy' : 'bg-gray-200'}`} />
                      )}
                    </div>
                    {/* Label */}
                    <p className={`text-sm pb-5 pt-0.5 transition ${isCompleted ? 'text-navy font-semibold' : 'text-gray-400'}`}>
                      {info.label || step}
                    </p>
                  </div>
                );
        ...
        {/* Items */}
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          <h3 className="text-sm font-bold text-navy mb-4">Items Ordered</h3>
          <div className="flex flex-col gap-3">
            {order.items?.map((item) => (
              <div key={item.id} className="flex items-center gap-3">
                <img
                  src={item.productImage || `https://via.placeholder.com/56x56/E8ECF8/0D1B5E?text=${item.productName?.[0]}`}
                  alt={item.productName}
                  className="w-14 h-14 rounded-xl object-cover bg-gray-50 flex-shrink-0"
                />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-navy line-clamp-2">{item.productName}</p>
        ...
        {/* Payment summary */}
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          <h3 className="text-sm font-bold text-navy mb-3">Payment Summary</h3>
          <div className="flex flex-col gap-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-500">Subtotal</span>
              <span className="font-semibold text-navy">₦{Number(order.subtotal).toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Delivery Fee</span>
              <span className="font-semibold text-navy">₦{Number(order.deliveryFee).toLocaleString()}</span>
            </div>
            {Number(order.discount) > 0 && (
              <div className="flex justify-between">
                <span className="text-navy">Discount ({order.promoCode})</span>
                <span className="font-semibold text-navy">-₦{Number(order.discount).toLocaleString()}</span>
              </div>
            )}
            <div className="flex justify-between pt-2 border-t border-gray-100 mt-1">
              <span className="font-bold text-navy">Total Paid</span>
              <span className="text-lg font-extrabold text-navy">₦{Number(order.total).toLocaleString()}</span>
            </div>
          </div>
        </div>
        ...
        {/* Delivery info */}
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          <h3 className="text-sm font-bold text-navy mb-3">Delivery Info</h3>
        ...
        {/* Payment info */}
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          <h3 className="text-sm font-bold text-navy mb-3">Payment Info</h3>
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              <IoCardOutline size={16} className="text-navy" />
              <span className="text-sm text-gray-600">Paystack · {order.paymentStatus}</span>
            </div>
            {order.paymentReference && (
              <div className="flex items-start gap-2">
                <IoReceiptOutline size={16} className="text-navy mt-0.5 flex-shrink-0" />
