...
export default function ProductDetailPage() {
  ...
  const images = product.images?.length > 0
    ...
    : [product.image || `https://via.placeholder.com/500x500/E8F5E9/1B5E20?text=${product.name?.[0]}`];
  ...
  return (
    <div className="pb-4">
      ...
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-2 text-sm font-semibold text-gray-600 hover:text-primary mb-4 transition"
      >
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-3">
          <div className="relative bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
            ...
            {product.prescriptionRequired && (
              <span className="absolute top-3 right-3 bg-primary text-white text-xs font-bold px-2 py-1 rounded-lg">Rx</span>
            )}
            {/* Nav controls */}
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Name */}
          <h1 className="text-2xl font-extrabold text-primary leading-tight">{product.name}</h1>
          ...
          <div className="flex items-baseline gap-3">
            <span className="text-3xl font-extrabold text-primary">₦{Number(product.price).toLocaleString()}</span>
            {hasDiscount && (
          ...
          {/* Quantity selector */}
          {inStock && (
            <div className="flex items-center gap-4 bg-primary-surface px-4 py-3 rounded-xl">
              <span className="text-sm font-semibold text-primary">Quantity</span>
              <div className="flex items-center gap-3 ml-auto">
                <button
                  onClick={() => setQty((q) => Math.max(1, q - 1))}
                  disabled={qty <= 1}
                  className="w-8 h-8 rounded-lg border-2 border-primary/30 flex items-center justify-center disabled:opacity-40 hover:border-primary transition font-bold text-primary"
                >−</button>
          ...
          {/* Dosage info card */}
          {product.dosage && (
            <div className="flex items-center gap-3 bg-primary-surface px-4 py-3 rounded-xl">
              <IoFlask size={20} className="text-primary flex-shrink-0" />
              <div>
                <p className="text-xs text-gray-500">Dosage</p>
                <p className="text-sm font-semibold text-primary">{product.dosage}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="mt-8 flex flex-col gap-6">
        ...
        {product.usageInstructions && (
          <div className="bg-white rounded-2xl p-6 border border-gray-100">
            <h3 className="text-base font-bold text-primary mb-2">Usage Instructions</h3>
        ...
        {product.reviews?.length > 0 && (
          <div className="bg-white rounded-2xl p-6 border border-gray-100">
            <h3 className="text-base font-bold text-primary mb-4">Customer Reviews</h3>
            <div className="flex flex-col gap-4">
                ...
                <div key={review.id} className="flex gap-3 pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-semibold text-primary">{review.user?.fullname}</span>
        ...
        {related.length > 0 && (
          <div>
            <h3 className="text-base font-bold text-primary mb-4">Related Products</h3>
