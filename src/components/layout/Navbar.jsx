      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
            ...
            {navLinks.map((l) => (
              <Link
                ...
                className={`text-sm font-semibold transition-colors ${
                  location.pathname === l.to ? 'text-cyan' : 'text-gray-600 hover:text-navy'
                }`}
              >
                {l.label}
              </Link>
          ...
          <div className="flex items-center gap-2">
            {/* Search */}
            <Link to="/search" className="p-2 hover:bg-navy-surface rounded-xl transition" aria-label="Search">
              <IoSearchOutline size={22} className="text-navy" />
            </Link>

            {/* Cart */}
            <Link to="/cart" className="relative p-2 hover:bg-navy-surface rounded-xl transition" aria-label="Cart">
              <IoCartOutline size={22} className="text-navy" />
              {isAuthenticated && itemCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-cyan text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center">
            ...
            {isAuthenticated ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-navy-surface transition"
                >
                    ...
                    <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-lg border border-gray-100 py-2 z-20">
                      ...
                      {[
                        ...
                        <Link
                          ...
                          className="flex items-center gap-3 px-4 py-2.5 hover:bg-navy-surface transition text-sm text-gray-700 font-medium"
                        >
              ...
              <div className="hidden sm:flex items-center gap-2">
                <Link to="/login" className="text-sm font-semibold text-navy hover:text-cyan transition px-3 py-2">
                  Sign In
                </Link>
            ...
            {/* Mobile menu toggle */}
            <button
              className="md:hidden p-2 hover:bg-navy-surface rounded-xl"
              onClick={() => setMenuOpen(!menuOpen)}
            >
              {menuOpen ? <IoCloseOutline size={22} className="text-navy" /> : <IoMenuOutline size={22} className="text-navy" />}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 flex flex-col gap-2">
          {navLinks.map((l) => (
            <Link
              ...
              className="py-2.5 text-sm font-semibold text-gray-700 hover:text-navy"
            >
              {l.label}
            </Link>
          ))}
          {!isAuthenticated && (
            <>
              <Link to="/login" onClick={() => setMenuOpen(false)} className="py-2.5 text-sm font-semibold text-navy">Sign In</Link>
